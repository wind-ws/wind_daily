<script lang = "ts" setup>
    ...
    import {get_priority_color_done, get_priority_color_undone, Todo} from "../../../ts_src/twin/user/user_todo";
    /// 双击编辑 title
    /// 左滑出现 隐藏按钮
    ...
    function click_switch_is() {//切换is状态,也表示 完成或未完成 的todo
    ...
    function dblclick_edit_title(){//双击编辑title ,检测是否脱离聚焦 服务 退出编辑
        ...
        function e_listener_edit(){
            ...
            if(original_height!=height){//弹出键盘
                console.log("come~")
            }else {//收起键盘
                console.log("oh~yes~")
...
<template>
  <div class = "w-full h-full" :style="{opacity:table.is?0.6:1}">
    <SwipeCell :right-width="80" :left-width="140">
      <template #left>
        <div class="flex h-full ignore-elements-ComRowTodo" style="width: 140px">
        ...
        <Row gutter="3" align="center">
          <Col span="3" justify="space-between" >
            <div class="self-center w-full h-full border-teal-700"
              ...
              <Icon.HeroSolid.StopIcon   v-if="table.is"
                                         class="inline-block h-7"
                                         :style="{color:get_priority_color_done(table.priority)}"/>
              <Icon.HeroOutline.StopIcon v-else
                                         class="inline-block h-7"
                                         :style="{color:get_priority_color_undone(table.priority)}"/>
            </div>
          </Col>
          <Col span="21">
            <div ref="title_block"
      ...
      <template #right>
        <div class="flex h-full ignore-elements-ComRowTodo" style="width: 100px">
          <Row gutter="3" align="center" class="h-full w-full">
...
<style scoped>
    .ignore-elements-ComRowTodo{}/*专门用于防止 被拖动,不可添加任何css属性*/
